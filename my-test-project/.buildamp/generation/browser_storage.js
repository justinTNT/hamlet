...
export function generateBrowserStorage() {
    ...
    const storageModelsPath = PROJECT_NAME ? path.join(process.cwd(), `app/${PROJECT_NAME}/models/storage`) : null;
    ...
    if (!fs.existsSync(storageModelsPath)) {
        console.log('📁 No models/storage directory found, skipping browser storage generation');
        return;
    }
