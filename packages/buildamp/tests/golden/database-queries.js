...
async function killGuest(id, host) {
    const result = await pool.query(
        'UPDATE guest SET deleted_at = extract(epoch from now()) * 1000 WHERE id = $1 AND host = $2 RETURNING *',
        [id, host]
    );
    return result.rows[0] || null;
}
...
async function killItemComment(id, host) {
    const result = await pool.query(
        'UPDATE item_comment SET deleted_at = extract(epoch from now()) * 1000 WHERE id = $1 AND host = $2 RETURNING *',
        [id, host]
    );
    return result.rows[0] || null;
}
...
async function killItemTag(id, host) {
    const result = await pool.query(
        'UPDATE item_tag SET deleted_at = extract(epoch from now()) * 1000 WHERE id = $1 AND host = $2 RETURNING *',
        [id, host]
    );
    return result.rows[0] || null;
}
...
async function killMicroblogItem(id, host) {
    const result = await pool.query(
        'UPDATE microblog_item SET deleted_at = extract(epoch from now()) * 1000 WHERE id = $1 AND host = $2 RETURNING *',
        [id, host]
    );
    return result.rows[0] || null;
}
...
async function killTag(id, host) {
    const result = await pool.query(
        'UPDATE tag SET deleted_at = extract(epoch from now()) * 1000 WHERE id = $1 AND host = $2 RETURNING *',
        [id, host]
    );
    return result.rows[0] || null;
}
