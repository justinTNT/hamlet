...
**Status: IMPLEMENTED (Schema-Driven)**
...
The schema-driven approach was implemented with:

### Backend
- `packages/hamlet-server/middleware/admin-api.js` - Full CRUD API
  - `GET /admin/api/schema` - Serves schema.json
  - `GET /admin/api/:resource` - List records
  - `GET /admin/api/:resource/:id` - Get single record
  - `POST /admin/api/:resource` - Create record
  - `PUT /admin/api/:resource/:id` - Update record
  - `DELETE /admin/api/:resource/:id` - Soft delete (kill)
- `packages/hamlet-server/middleware/admin-auth.js` - Token authentication
  - Reads `HAMLET_ADMIN_TOKEN` from environment
  - Accepts token via Bearer header, query param, or cookie

### Frontend
- `app/horatio/admin/src/Main.elm` - Schema-driven Elm app
  - Fetches schema.json on init
  - Dynamically renders sidebar from schema tables
  - Dynamically renders table views from schema fields
  - Dynamically renders forms with type-appropriate inputs
  - Foreign key fields show references hint
  - No code generation needed when models change

### Schema
- `app/horatio/server/.hamlet-gen/schema.json` - Generated by BuildAmp
  - Contains all table definitions with fields, types, relationships
  - Updated automatically when `buildamp gen` runs
