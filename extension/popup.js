...
var $author$project$Api$Port$send = F2(
	function (toMsg, req) {
		var callback = function (result) {
			if (result.$ === 'Ok') {
				var value = result.a;
				var _v1 = A2($elm$json$Json$Decode$decodeValue, req.decoder, value);
				if (_v1.$ === 'Ok') {
					var decoded = _v1.a;
					return toMsg(
		...
		return A3($author$project$Api$Port$Send, req.endpoint, req.body, callback);
	});
...
var $author$project$Api$Port$update = F3(
	function (config, msg, model) {
		if (msg.$ === 'Send') {
			...
			var newPending = A3($elm$core$Dict$insert, correlationId, callback, model.pending);
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{counter: newCounter, pending: newPending}),
			...
			var envelopeDecoder = A4(
				...
				F3(
					function (c, b, e) {
						return {body: b, correlationId: c, error: e};
					}),
			...
			if (_v1.$ === 'Ok') {
				var correlationId = _v1.a.correlationId;
				var body = _v1.a.body;
				var error = _v1.a.error;
				var _v2 = A2($elm$core$Dict$get, correlationId, model.pending);
				if (_v2.$ === 'Just') {
					var callback = _v2.a;
					var result = function () {
						if (error.$ === 'Just') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								pending: A2($elm$core$Dict$remove, correlationId, model.pending)
							}),
						cmd);
				} else {
