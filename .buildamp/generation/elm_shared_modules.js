...
export async function generateElmSharedModules() {
    ...
    const outputDir = 'app/horatio/server/generated';
    ...
    const modules = [
        { name: 'Database.elm', content: generateDatabaseModule() },
        { name: 'Events.elm', content: generateEventsModule() },
 ...
 */
function generateDatabaseModule() {
    // Parse actual Rust models to generate correct Elm types
    const dbModels = parseRustDbModels();
    
    return `module Generated.Database exposing (..)
...
function generateEventsModule() {
    // TODO: Parse src/models/events/*.rs to auto-generate event types
...
function generateServicesModule() {
    return `module Generated.Services exposing (..)
 ...
 */
function parseRustDbModels() {
    const models = [];
    const dbPath = 'src/models/db/';
...
function rustTypeToElmType(rustType, knownModels = []) {
    ...
    if (rustType.startsWith('Option<')) {
        const innerType = rustType.match(/Option<(.+)>/)[1];
...
function generateFieldEncoder(field, isOptional = false) {
    const baseEncoder = generateBasicEncoder(field.elmType);
