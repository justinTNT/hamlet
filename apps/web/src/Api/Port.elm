    ...
    case msg of
        Send endpoint body callback ->
            let
                ...
                correlationId = String.fromInt newCounter
                ...
                newPending = Dict.insert correlationId callback model.pending
            in
...
send toMsg req =
    let
        ...
        callback result =
            case result of
                Ok value ->
                    case Decode.decodeValue req.decoder value of
                        Ok decoded -> toMsg (Ok decoded)
                        Err err -> toMsg (Err (Decode.errorToString err))
